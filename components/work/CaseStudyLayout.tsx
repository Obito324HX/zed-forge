...
export function CaseStudyLayout({ study }: { study: CaseStudy }) {
 ...
 <Link href="/#work" className="focus-ring inline-flex items-center gap-2 text-sm text-ff-muted hover:text-ff-text">
 ...
 <div className="relative mt-10 min-h-[360px] overflow-hidden border border-ff-border shadow-glow">
 <Image src={study.image} alt={`${study.title} hero screenshot`} fill className="object-cover" priority />
 </div>
 <section className="mt-8 grid gap-4 md:grid-cols-3">
 {study.meta.map((item) => (
 <div key={item.label} className="border border-ff-border bg-ff-surface p-5">
 ...
 <div key={feature.name} className="border border-ff-border bg-ff-surface p-5">
 ...
 <div key={image} className="relative min-h-64 overflow-hidden border border-ff-border">
